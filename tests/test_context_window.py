...
from koko_pi_agent.client import resolve_context_window
from koko_pi_agent.config import ProviderConfig
from koko_pi_agent.validator import (
    ConfigError,
...
class TestAutoFetch:
    ...
    async def test_fetch_success_is_cached_and_used(self):
        ...
        with patch("koko_pi_agent.client.create_client", return_value=fake) as mk:
    ...
    async def test_fetch_raises_degrades_to_mapping_table(self):
        ...
        with patch("koko_pi_agent.client.create_client", return_value=fake):
            # 不应抛出异常。
            await resolve_context_window(p)
        # 对 claude 回退到映射表。
    ...
    async def test_client_construction_failure_degrades(self):
        ...
        p = _provider(model="claude-sonnet-4-6")
        with patch(
            "koko_pi_agent.client.create_client",
    ...
    async def test_non_anthropic_provider_is_not_fetched(self):
        ...
        with patch("koko_pi_agent.client.create_client") as mk:
            await resolve_context_window(p)
