...
from koko_pi_agent.teams.progress import TeammateProgress, random_verb

if TYPE_CHECKING:
    from koko_pi_agent.agent import Agent
    from koko_pi_agent.conversation import ConversationManager
    from koko_pi_agent.teams.models import TeammateInfo
        ...
        for m in mailbox.consume(member_name):
            ...
            if (m.type == protocol.PLAN_APPROVAL_RESPONSE
                ...
                if protocol.approved(m):
                    # 批准后切回正常权限，队友可以改文件了
                    from koko_pi_agent.permissions.modes import PermissionMode
    ...
    async def _run() -> str:
        ...
        try:
            if conversation is not None:
                ...
                from koko_pi_agent.conversation import ConversationManager as CM
                conv = CM()
