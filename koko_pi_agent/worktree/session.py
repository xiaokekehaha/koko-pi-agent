...
def _session_path(koko_dir: Path) -> Path:
    return koko_dir / SESSION_FILENAME
...
def load_worktree_session(koko_dir: Path) -> WorktreeSession | None:
    path = _session_path(koko_dir)
    if not path.exists():
        return None
    try:
