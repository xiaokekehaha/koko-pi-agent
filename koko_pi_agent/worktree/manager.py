...
from koko_pi_agent.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".koko" / "worktrees"
        )
        self._koko_dir = Path(repo_root) / ".koko"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._koko_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._koko_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._koko_dir)
        if session is None:
            return None
